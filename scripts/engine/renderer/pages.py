...
def render_projects_index_page(site: dict[str, str], system: dict[str, Any], posts: list[dict[str, Any]], projects: list[dict[str, Any]], documents: list[dict[str, Any]], i18n: dict[str, Any], locale: str) -> str:
    ...
    <div class="project-flow-shell">
      <div class="flow-toolbar"><span class="flow-kicker" data-i18n="nav.projects">{html.escape(translate(i18n, locale, "nav.projects", "projetos"))}</span><span class="flow-hint" data-i18n="pages.projects.flow_hint">{html.escape(translate(i18n, locale, "pages.projects.flow_hint", "Explore a visualização interativa das publicações."))}</span></div>
...
def render_document_page(site: dict[str, str], system: dict[str, Any], document: dict[str, Any], i18n: dict[str, Any], locale: str) -> str:
    ...
    content = f"""{breadcrumbs}<section class="page-grid"><article class="document-shell prose"><header class="post-header"><p class="section-kicker" data-i18n="pages.document.kicker">{html.escape(translate(i18n, locale, "pages.document.kicker", "document"))}</p><h1>{html.escape(document['title'])}</h1><p class="post-summary">{html.escape(document['summary'])}</p></header>{render_markdown(document.get('body', ""))}</article>{sidebar}</section>"""
