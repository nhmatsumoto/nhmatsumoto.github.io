/**
 * Section Canvas Reader
 * Binary-tree traversal of project sections — pure Canvas 2D, no dependencies.
 *
 * Data format (sections-data JSON):
 * [{ id, type, title, content, animation, children: [id, ...] }, ...]
 *
 * Animations: fade | slide_right | zoom | typewriter | matrix_rain | expand_node
 */
;(function () {
  'use strict'
  ...
  const spring  = (p) => {
    // overshoot spring: slightly exceeds 1 then settles
    const s = easeOut3(p)
  ...
  // ── Animation drivers ──────────────────────────────────────────────────────
  // Each mutates `rs` (render state) based on progress p ∈ [0, 1]
  ...
  class SectionNode {
    constructor(section) {
      ...
      this.entering = false
      this.animStartT = 0
      this.animDuration = 0.85  // seconds
      ...
      // Pre-generate rain chars (katakana + symbols)
      this._rainCols = []
      for (let i = 0; i < 16; i++) {
    ...
    drawActive(ctx, t) {
      ...
      // ── Matrix rain overlay (pre-reveal phase) ─────────────────────────
    ...
    // ── Mini card (child option or visited history) ─────────────────────────
    drawMini(ctx, role) {
      // role: 'child' | 'history'
  ...
  function init(container, sections) {
    ...
    let W = 0, H = 0
    let currentId = null          // active section id
    let visitedStack = []         // ids of visited nodes in order
    let childViews = []           // SectionNode[] for current children (mini)
    let histViews  = []           // SectionNode[] for visited nodes (mini, breadcrumb)
    let activeNode = null         // SectionNode for the current section (large)
    ...
    function activateSection(id) {
      ...
      // Build history views (most recent first, max 6)
    ...
    function goBack() {
      ...
      // Remove the one we just added in activateSection (it was already in stack)
      visitedStack.pop()
    }
    ...
    function draw(ts) {
      ...
      // ── Connection lines: active → children ───────────────────────────
      if (activeNode && childViews.length) {
        for (const cv of childViews) {
          ...
          // Animated dot flowing along path
          if (cv.hover) {
            const prog = ((t * 0.7) % 1)
