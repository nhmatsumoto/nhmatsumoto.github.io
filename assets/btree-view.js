/**
 * Binary Tree View Engine — nhmatsumoto-blog-engine
 *
 * Loads the search index, builds a balanced BST sorted by date,
 * and renders it as an interactive canvas overlay.
 *
 * Trigger: floating button (bottom-right). Auto-opens on page-home.
 */
;(function () {
  'use strict'
  ...
  // ── Tree layout (level-order, balanced horizontal spread) ───────────────────
  ...
  function drawNode(ctx, node, t) {
    ...
    // Current page indicator (pulse dot)
    if (node.current) {
  ...
  function drawConnection(ctx, parent, child) {
    ...
    // Animated flow dot on active connection
    if (isActive) {
  ...
  function createTrigger() {
    // Don't add on homepage — it auto-opens there
  ...
  async function init(searchIndexUrl) {
    ...
    function open() {
      ...
      // Cascade reveal from center outward
    ...
    if (document.body.classList.contains('page-home')) {
      // slight delay to let the page paint first
      setTimeout(open, 200)
    }
    ...
    function draw(ts) {
      ...
      // Update nodes (reveal cascade + springs)
      let revealBudget = 3  // reveal at most N nodes per frame
      allNodes.forEach(n => {
      ...
      // Draw nodes (back-to-front by alpha)
      allNodes
        .slice()
